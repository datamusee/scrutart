# copie de setTagsFromWikidataDepictsToPiwigoImages.py avec query wikidata modifiée de depicts à movement
# et enregistrement e movement dans l'image
import re
import requests
    ...
    for creator in creatorsToProcess:
        ...
        if dictim:
            ...
            for uri, im in dictim.items():
                if ("posted" in im) and (im["posted"]) and (not "movements" in dictim[uri]):
                    # find genre(s) avec label in wikidata for the image
                    print(uri)
                    crtquery = queryMovements.replace("__ARTWORKENTITY__", uri)
                    ...
                    wtags = res.bindings
                    for wtag in wtags:
                        label = wtag["movementLabel"].value
                    ...
                    if tagIds:
                        ...
                        if imId:
                            if addTagsToPiwigoImage(session, tagIds, imId):
                                if not "movements" in dictim[uri]: dictim[uri]["movements"]=[]
                                dictim[uri]["movements"].extend(tagIds)
                    time.sleep(sleep)
                    if idxsav>=freqsav:
                        idxsav = 0
